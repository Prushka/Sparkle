...
tracks. Its HEVC encoder is `nvenc_h265_10bit` at `slowest`; the equivalent FFmpeg
preset is `p7`. Sparkle-Transcoder currently uses software SVT AV1 preset 4; this
mode intentionally uses NVENC AV1 at the corresponding hardware `p7` preset.
...
- Six-second independently encoded fragments form one seekable HLS/fMP4 timeline.
  Video and audio playlists are separate, so playing audio does not download video
  a second time. Distant seeks generate only the requested segments.
...
manifest, video/audio playlists, their init files, numbered fragment/subtitle
